 ...
 * @param {boolean} isToggled - Indicates whether the button is in the toggled state.
...
export default function ToggleButton({ className = "", labels, content, isToggled, onClick }) {
    return (
        <Button className={`togglebutton ${className}`} type="button" onClick={onClick}>
            {isToggled ?
                <>
                    {content.after}
                    {labels && labels.after ? <span className="togglebutton-text">{labels.after}</span> : <></>}
                </>
            :
                <>
                    {content.before}
                    {labels && labels.before ? <span className="togglebutton-text">{labels.before}</span> : <></>}
                </>
            }
        </Button>
    );
